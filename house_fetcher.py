...
@cli.command()
@click.option('--year', help='Year of document to download.', type=click.IntRange(2004, datetime.datetime.now().year))
@click.option('--document', help='Document to download', type=click.Choice(["Registrations", "1stQuarter", "2ndQuarter", "3rdQuarter", "4thQuarter", "MidYear", "YearEnd"]))
...
def download(year, document, file):
    ...
    view_state_generator = r.html.find("#__VIEWSTATEGENERATOR", first=True).attrs['value']
    ...
    # Find The File
    files = [e.attrs['value'] for e in r.html.xpath("//option") if formatted_filename in e.attrs['value'] ]
    if(len(files) == 0):
    ...
    with session.post(DOWNLOAD_URL, data={
